...
public class Dungeon_8 implements Screen {
    ...
    public Dungeon_8(MyGdxGame game, Array<Texture> ISlots, int floor, ArrayList<Integer> Dungeon_list, float health, float level, int[] itemsCollect, boolean first1, float[] hungers, int[] stats) {
        this.game = game;
        this.ISlots = ISlots;
    ...
    private void worldcheck() {
        Dungeon_list.add(8);
        map1 = new TmxMapLoader( ).load("dungeons/dungeon_8.tmx");
        tiledMapRenderer = new OrthogonalTiledMapRenderer(map1, 2);
        ...
        player = new Player(world, 644, 900);
    ...
    private void moveinvertoy() {
        for (int meow = 0; meow < 9; meow++) {
            ...
            batch.begin( );
            if (first)
                batch.draw(invertoy_backgroundT, Invertoyparts[meow].getPosition( ).x - Invertoy.BOX_SIZE_width_part / Worldbegginerplace.PIXEL_PER_METER / 2, Invertoyparts[meow].getPosition( ).y - Invertoy.BOX_SIZE_hight_part / Worldbegginerplace.PIXEL_PER_METER / 2, Invertoy.BOX_SIZE_width_part / Playscreen.PIXEL_PER_METER, Invertoy.BOX_SIZE_hight_part / Playscreen.PIXEL_PER_METER);
            batch.end( );
        }
        batch.begin( );
        for (int mmm = 0; mmm < ISlots.size && !first; mmm++) {
