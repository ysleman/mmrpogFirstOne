...
public class Dungeon_10 implements Screen {
    ...
    private Body Stair_Down;

    public Dungeon_10(MyGdxGame game, Array<Texture> ISlots, int floor, ArrayList<Integer> Dungeon_list, float health, float level, int[] itemsCollect, boolean first1, float[] hungers, int[] stats){
        this.game=game;
        this.ISlots=ISlots;
        this.Health=health;
        this.Level=level;
    ...
    private void worldcheck() {
        Dungeon_list.add(10);
        map1=new TmxMapLoader().load("dungeons/dungeon_10.tmx");
        tiledMapRenderer = new OrthogonalTiledMapRenderer(map1,2);
        mapLayer= (TiledMapTileLayer) map1.getLayers().get(1);
        TiledObjectlayer.parseTiledObjectLayer1(world,map1.getLayers().get(1).getObjects());
        ...
        player = new Player(world,1931,2926);
    ...
    public void render(float delta) {
        ...
        for(i3=0;i3<SkeletonBody.size;i3++) {
            ...
            if((int)player1.getPosition().y/10==(int)SkeletonBody.get(i3).getPosition().y/10) {
                ...
                if((int)player1.getPosition().x<(int)SkeletonBody.get(i3).getPosition().x&&enableanimatonSkeleton.get(i3))batch.draw((TextureRegion) skeleton_walk_left.getKeyFrame(timeloop4,true),SkeletonBody.get(i3).getPosition().x-Skeleton_Width,SkeletonBody.get(i3).getPosition().y-Skeleton_Height,SkeletonSize,SkeletonSize);
                if((int)player1.getPosition().x>(int)SkeletonBody.get(i3).getPosition().x&&enableanimatonSkeleton.get(i3))batch.draw((TextureRegion) skeleton_walk_Right.getKeyFrame(timeloop4,true),SkeletonBody.get(i3).getPosition().x-Skeleton_Width,SkeletonBody.get(i3).getPosition().y-Skeleton_Height,SkeletonSize,SkeletonSize);
            }
...

        {
            ...
            if (!Died_Ghost.get(i5)) {
                //right
                if (hit_Ghost.get(i5) && (length_Ghost.get(i5) >= 0) && (length_Ghost.get(i5) <= 70) && hitforGhostEnable.get(i5) && player1.getPosition( ).x > GhostBody.get(i5).getPosition( ).x) {
    ...
    private void ChangeScreenHoles() {
        ...
        //Todo: make the return to surfce by stairs
    }
    ...
    private void moveinvertoy() {
        for(int meow=0;meow<9;meow++) {
            Invertoyparts[meow].setLinearVelocity(player1.getLinearVelocity( ).x, player1.getLinearVelocity( ).y);
            batch.begin( );
            if(first) batch.draw(invertoy_backgroundT, Invertoyparts[meow].getPosition( ).x - Invertoy.BOX_SIZE_width_part / Worldbegginerplace.PIXEL_PER_METER / 2, Invertoyparts[meow].getPosition( ).y - Invertoy.BOX_SIZE_hight_part / Worldbegginerplace.PIXEL_PER_METER / 2, Invertoy.BOX_SIZE_width_part / Playscreen.PIXEL_PER_METER, Invertoy.BOX_SIZE_hight_part / Playscreen.PIXEL_PER_METER);
            batch.end();
        }
        batch.begin();
        for(int mmm=0;mmm<ISlots.size&&!first;mmm++){
            batch.draw(ISlots.get(mmm), Invertoyparts[mmm].getPosition( ).x - Invertoy.BOX_SIZE_width_part / Worldbegginerplace.PIXEL_PER_METER / 2, Invertoyparts[mmm].getPosition( ).y - Invertoy.BOX_SIZE_hight_part / Worldbegginerplace.PIXEL_PER_METER / 2, Invertoy.BOX_SIZE_width_part / Playscreen.PIXEL_PER_METER, Invertoy.BOX_SIZE_hight_part / Playscreen.PIXEL_PER_METER);
            if(ItemsCollect[0]<10){
                if(ISlots.get(mmm).getHeight()==(rat_meat).getHeight())
                    font.draw(batch,""+ItemsCollect[0],Invertoyparts[mmm].getPosition().x+3,Invertoyparts[mmm].getPosition().y+8);

            }
            if(ItemsCollect[0]>=10){
                if(ISlots.get(mmm).getHeight()==(rat_meat).getHeight())
                    font.draw(batch,""+ItemsCollect[0],Invertoyparts[mmm].getPosition().x-6,Invertoyparts[mmm].getPosition().y+8);

            }

            if(ItemsCollect[1]<10){
                if(ISlots.get(mmm).getHeight()==(sheep_meat).getHeight())
                    font.draw(batch,""+ItemsCollect[1],Invertoyparts[mmm].getPosition().x+3,Invertoyparts[mmm].getPosition().y+8);

            }

            if(ItemsCollect[1]>=10){
                if(ISlots.get(mmm).getHeight()==(sheep_meat).getHeight())
                    font.draw(batch,""+ItemsCollect[1],Invertoyparts[mmm].getPosition().x-6,Invertoyparts[mmm].getPosition().y+8);
            }
            if(ItemsCollect[2]<10){
                if(ISlots.get(mmm).getHeight()==(woodT).getHeight())
                    font.draw(batch,""+ItemsCollect[2],Invertoyparts[mmm].getPosition().x+3,Invertoyparts[mmm].getPosition().y+8);
            }
            if(ItemsCollect[2]>=10){
                if(ISlots.get(mmm).getHeight()==(woodT).getHeight())
                    font.draw(batch,""+ItemsCollect[2],Invertoyparts[mmm].getPosition().x-6,Invertoyparts[mmm].getPosition().y+8);
            }

            if(ItemsCollect[3]<10){
                if(ISlots.get(mmm).getHeight()==(rockT).getHeight())
                    font.draw(batch,""+ItemsCollect[3],Invertoyparts[mmm].getPosition().x+3,Invertoyparts[mmm].getPosition().y+8);
            }
            if(ItemsCollect[3]>=10){
                if(ISlots.get(mmm).getHeight()==(rockT).getHeight())
                    font.draw(batch,""+ItemsCollect[3],Invertoyparts[mmm].getPosition().x-6,Invertoyparts[mmm].getPosition().y+8);
            }

            if(ItemsCollect[4]<10)
                if(ISlots.get(mmm).getHeight()==(FishT).getHeight()){
                    font.draw(batch,""+ItemsCollect[4],Invertoyparts[mmm].getPosition().x+3,Invertoyparts[mmm].getPosition().y+8);
                }
            if(ItemsCollect[4]>=10)
                if(ISlots.get(mmm).getHeight()==(FishT).getHeight()){
                    font.draw(batch,""+ItemsCollect[4],Invertoyparts[mmm].getPosition().x-6,Invertoyparts[mmm].getPosition().y+8);
                }
    ...
    private void check() {
        for (int i=0;i<160;i++)for(int j=0;j<180;j++){
