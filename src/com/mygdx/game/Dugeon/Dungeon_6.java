...
public class Dungeon_6 implements Screen {
    ...
    public Dungeon_6(MyGdxGame game, Array<Texture> ISlots, int floor, ArrayList<Integer> Dungeon_list, float health, float level, int[] itemsCollect, boolean first1, float[] hungers, int[] stats){
        this.game=game;
        this.ISlots=ISlots;
        this.Health=health;
        this.Level=level;
    ...
    private void worldcheck() {
        Dungeon_list.add(6);
        map1=new TmxMapLoader().load("dungeons/dungeon_6.tmx");
        tiledMapRenderer = new OrthogonalTiledMapRenderer(map1,2);
        mapLayer= (TiledMapTileLayer) map1.getLayers().get(1);
        System.out.println(map1.getLayers().get(3).getObjects().getCount());
    ...
    private void check() {
        for (int i=0;i<52;i++)for(int j=0;j<120;j++){
